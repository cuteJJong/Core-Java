...
2. Data를 Stream으로 처리하기 위해서는 먼저 Stream 객체를 생성해야 합니다.
3. Stream API의 3가지 종류
    1)Stream 생성
    2)생성된 Stream을 가공하는 중간 연산
    3)최종적으로 결과를 만들어내는 최종 연산
4. 연산의 결과가 Stream으로 리턴되면 중간연산이라고 한다.반면, 연산의 결과가 Stream이 아닌 형태로 나타나면 최종 연산이라 한다.
5. 중간 연산의 결과가 Stream으로 리턴이 되기 때문에, Stream 연산을 계속 이어서 chain 형태로 이용할 수 있다.
