...
public class StreamDemo3 {
    public static void main(String[] args) {
        ...
        sStream.filter(str -> str.length()>= 5 ).forEach(s -> System.out.println(s + "\t"));     //length 가 5글자 같거나 큰

        List<Customer> list = new ArrayList<Customer>();
        Customer c1 = new Customer("한지민", 28);
        list.add(c1);
        Customer c2 = new Customer("김지민", 38);
        list.add(c2);
        Customer c3 = new Customer("박지민", 48);
        list.add(c3);
        Customer c4 = new Customer("홍지민", 58);
        list.add(c4);
